...
import Styles from "./QuantityPage.module.css";
    ...
    tableData.filter((item) => {
      if (item.id == id) {
        if (item.price != newValue) {
          ...
          if (
            editedData.filter((itemE) => itemE.quantity != item.quantity)
              .length > 0
          ) {
            setEditedData(
              editedData.map((itemN) => {
                if (itemN.id == id) {
      ...
      for (let i = 0; i < editedData.length; i++) {
        if (document.getElementById(`input_price_${editedData[i].id}`)) {
          ...
          if (
            ...
            tableData.filter((item) => item.id == editedData[i].id)[0].quantity
          ) {
            document.getElementById(
          ...
          if (
            ...
            setEditedData(
              editedData.filter((item) => item.id != editedData[i].id)
            );
          }
        }
      }
    ...
    tableData.filter((item) => {
      if (item.id == id) {
        if (item.quantity != newValue) {
          ...
          if (
            editedData.filter((itemE) => itemE.price != item.price).length > 0
          ) {
            setEditedData(
              editedData.map((itemN) => {
                if (itemN.id == id) {
  ...
  return (
    <div>
      <Helmet>
        ...
        <meta
          name="description"
          content="موجودی محصولات %REACT_APP_WEBSITE_NAME%"
        />
      </Helmet>
                ...
                for (let i = 0; i < editedData.length; i++) {
                  GetProduct(editedData[i].id).then((res) => {
                    if (res.status == 200) {
                      ...
                      newProduct.price.amount = editedData.filter(
                        (item) => item.id == newProduct.id
                      )[0].price;
                      newProduct.count = Number(
                        editedData.filter((item) => item.id == newProduct.id)[0]
                          .quantity
                      );
                      UpdateProduct(editedData[i].id, newProduct).then(
                        (res) => {
                          if (res.status == 200) {
                            setSaveButtonStatus(false);
                            setTableData(
