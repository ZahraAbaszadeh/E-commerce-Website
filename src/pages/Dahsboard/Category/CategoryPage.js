...
import Styles from "./CategorySytlePage.module.css";
          ...
          if (res.status === 201) {
            setTableData((tableData) => [
              ...tableData,
              {
                ...
                icon:
                  process.env.REACT_APP_BASE_URL + "/files/" + res.data.icon,
              },
            ]);
            setShowModal(false);
            setEditMode(false);
      ...
      if (e.target.icon.files[0] == undefined) {
        ...
        EditCategory(category).then((res) => {
          if (res.status === 200) {
            ...
            tableData[index].icon =
              process.env.REACT_APP_BASE_URL + "/files/" + res.data.icon;
            setTableData([...tableData]);
            setShowModal(false);
            setEditMode(false);
            swal("ویرایش شد!", "دسته بندی ویرایش شد!", "success");
          }
        });
      } else {
        let formData = new FormData();
  ...
  return (
    <div className="contentWithHeaderAndFooter">
      <Helmet>
        ...
        <meta
          name="description"
          content="لیست دسته بندی های %REACT_APP_WEBSITE_NAME%"
        />
      </Helmet>
      ...
      <DashboardLayout>
        ...
        >
          <div className={Styles.ModalHeader}>
            {editMode ? <h1>ویرایش دسته بندی</h1> : <h1>افزودن دسته بندی</h1>}
            <Button
              text="بستن"
              type="danger"
              size="small"
