...
import { userLoginValidationSchema } from "../../validations/userLogin";
...
export const LoginPage = (props) => {
  ...
  const loginUser = async (event) => {
    ...
    const validData = await userLoginValidationSchema
      .validate(data)
      .catch((err) => {
    ...
    if (validData != false) {
      ...
      try {
        ...
        e.response.status == 400
          ? swal("خطا", "کاربر مورد نظر یافت نشد", "error")
      ...
      <div className={Styles.loginBox}>
        ...
        <form onSubmit={loginUser}>
          <div>
            ...
            />
            {passwordError && (
              <p className={Styles.errorMessage}>{passwordError.message}</p>
            )}
            <Button
              type="success"
              size="large"
